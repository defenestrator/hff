<spark-payment-method-stripe :user="user" :team="team" :billable-type="billableType" inline-template>
    <div>
        ...
        @if (Spark::collectsEuropeanVat())
            @include('settings.payment-method.update-vat-id')
        @endif

        <!-- Update Card -->
        @include('settings.payment-method.update-payment-method-stripe')

        <div>
            <div v-if="billable.stripe_id">
                <!-- Redeem Coupon -->
                @include('settings.payment-method.redeem-coupon')
            </div>
        </div>
    </div>
